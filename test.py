...
from learnpy.functions import relu, sigmoid, tanh, elu, cross_entropy_cost
...
def first_example():
    ...
    # np.random.seed(0)
    X, y = classify_spiral_data(1000, 2, 25)
    # X, y = classify_circular_data(600, 0.1, 25)
    # X, y = classify_XOR_data(600, 0.1, 8, 0.3)
    ...
    if should_save == True and os.path.exists(eliska):
        clf = MLP.load(eliska)
        clf.learning_rate = 0.003
        clf.regularization_rate = 0.3
    else:
        clf = MLP(layers=(X.shape[1], 8, 6, 2),
            activation=sigmoid,
            learning_rate=0.3,
            tolerance=10e-12,
            regularization='l1',
    ...
    print()
    print("MLP({layers: %s, activation=%s})" % (str(clf.layers), str(clf.activation)))
